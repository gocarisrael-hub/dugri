...
- In `index.js`: the collections routes (incl. `/pawns`, `/pawn-view`, `/players`, `/pawn-card`, `/proof*`), `/api/preview`, `/api/wordlist-options`, `/api/wordlist-preview`, `/api/admin/wordlists*`, `/api/admin/collections/:id/generate`, `/proof`, `/press` + the generator spawn logic, redetect and calibration routes
