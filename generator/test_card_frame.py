...
# --------------------------------------------------------------------------
# where the frame sits: centring the card on its page
# --------------------------------------------------------------------------

def _margins(svg, cell=CARD):
    f = card_frame.frame(svg, cell)
    return (f["x"] - cell[0], cell[2] - (f["x"] + f["w"]),
            f["y"] - cell[1], cell[3] - (f["y"] + f["h"]))


BACKGROUND = '<rect x="0" y="0" width="223.92" height="312" fill="#cae6f5"/>'
# Grapefruit's back, in miniature: its frame is 1.94 units WIDER than its own
# fronts and the surplus hangs off the right, so the printed border sits 0.33mm
# nearer one edge than the other.
OFF_CENTRE = ('<rect x="24.34" y="22.44" width="177.12" height="266.93" '
              'rx="7.5" fill="none" stroke="#711d20" stroke-width="1.5"/>')


def test_a_card_drawn_off_centre_is_slid_back_onto_it():
    svg = _card(BACKGROUND + OFF_CENTRE)
    left, right, _top, _bottom = _margins(svg)
    assert round(left - right, 2) == 1.88              # the shipped back's error
    assert card_frame.centring_shift(
        card_frame.centred(svg, CARD), CARD) == (0.0, 0.0)


def test_centring_moves_the_WHOLE_card_not_just_its_frame():
    """Frame and background are one drawing; sliding the frame alone would only
    move the error to the artwork it is drawn around."""
    out = card_frame.centred(_card(BACKGROUND + OFF_CENTRE), CARD)
    assert 'transform="translate(-0.94,0.09)"' in out
    # everything the card draws, inside the one <g>. rindex for the background:
    # the underlay painted under the page is the same rect, written again.
    assert out.index("translate") < out.index(OFF_CENTRE)
    assert out.index("translate") < out.rindex(BACKGROUND)


def test_the_strip_a_shift_bares_is_filled_with_the_cards_own_background():
    """A customer's deck is one card per page with no bleed to spare, so the
    0.34mm the shift uncovers lands on the cut — as a white line, unless the
    background it moved off is painted back under the page first."""
    out = card_frame.centred(_card(BACKGROUND + OFF_CENTRE), CARD)
    under = re.search(r"<svg[^>]*>(<rect[^>]*/>)", out).group(1)
    assert 'fill="#cae6f5"' in under                    # the card's own colour
    assert card_frame.page_fill(out, CARD) == "#cae6f5"  # and it covers the page
    assert out.index(under) < out.index("translate")    # UNDER the artwork


def test_the_background_is_the_TOPMOST_full_page_fill():
    """A Canva export lays white down first and the design's colour over it."""
    svg = _card('<path fill="#ffffff" d="M 0 0.05 L 224.17 0.05 '
                'L 224.17 311.95 L 0 311.95 Z"/>'
                '<path fill="#cae6f5" d="M 0 0.05 L 224.17 0.05 '
                'L 224.17 311.95 L 0 311.95 Z"/>' + OFF_CENTRE)
    assert card_frame.page_fill(svg, CARD) == "#cae6f5"


def test_a_pattern_is_not_a_colour_and_cannot_fill_anything():
    svg = _card('<rect x="0" y="0" width="223.92" height="312" '
                'fill="url(#grapefruits)"/>' + OFF_CENTRE)
    assert card_frame.page_fill(svg, CARD) is None


def test_a_card_with_no_flat_background_may_only_be_nudged(capsys):
    """Nothing can fill what the shift bares, so a card slightly off centre is
    kept: that is a much smaller fault than a white line down its edge."""
    svg = _card('<rect x="0" y="0" width="223.92" height="312" '
                'fill="url(#grapefruits)"/>' + OFF_CENTRE)
    assert card_frame.centred(svg, CARD, what="a patterned card") is svg
    assert "a patterned card" in capsys.readouterr().err
    # ...but a shift thinner than a printer dot bares nothing worth filling.
    hair = _card('<rect x="0" y="0" width="223.92" height="312" '
                 'fill="url(#grapefruits)"/>'
                 '<rect x="24.34" y="22.44" width="175.18" height="266.92" '
                 'fill="none" stroke="#711d20" stroke-width="1.5"/>')
    assert 0 < abs(card_frame.centring_shift(hair, CARD)[1]) <= card_frame.BARE_MAX
    assert card_frame.centred(hair, CARD) is not hair


def test_a_card_already_centred_comes_back_untouched():
    svg = _card('<rect x="24.34" y="22.5" width="175.18" height="267" '
                'fill="none" stroke="#711d20" stroke-width="1.5"/>')
    assert card_frame.centring_shift(svg, CARD) == (0.0, 0.0)
    assert card_frame.centred(svg, CARD) is svg


def test_a_card_off_by_more_than_a_drafting_slip_is_left_as_drawn(capsys):
    """Past CENTRE_MAX a card is asymmetric ON PURPOSE, and sliding it would be
    a redesign — and would bare more of the page than the bleed can hide."""
    svg = _card('<rect x="4" y="22.44" width="175.18" height="266.93" '
                'fill="none" stroke="#711d20" stroke-width="1.5"/>')
    assert card_frame.centred(svg, CARD, what="a wonky card") is svg
    assert "a wonky card" in capsys.readouterr().err


def test_a_card_that_draws_no_frame_cannot_be_centred():
    svg = _card('<rect x="0" y="0" width="223.92" height="312" fill="#cfe4f2"/>')
    assert card_frame.centring_shift(svg, CARD) == (0.0, 0.0)
    assert card_frame.centred(svg, CARD) is svg


def test_every_shipped_grapefruit_card_comes_out_centred():
    """The regression this exists for: card 1 draws its frame 1.94 units wider
    than its own fronts and hangs the surplus off the right, printing the back's
    border 0.33mm nearer one edge than the other. Measured against the DECK's
    page — the shared viewBox every design is inlined into — and not against each
    card's own canvas, which for card 9 is a different width again."""
    off = {}
    for i in range(1, 10):
        svg = card_assets.read_svg(config.card_path("grapefruit", i))
        before = card_frame.centring_shift(svg, CARD)
        assert card_frame.centring_shift(
            card_frame.centred(svg, CARD), CARD) == (0.0, 0.0), i
        if before != (0.0, 0.0):
            off[i] = before
    assert round(off[1][0], 2) == -0.94             # the back, hung off the right
    assert round(off[9][0], 2) == 0.30              # card 9, drawn on a wider canvas


def test_the_deck_registers_every_design_centred(tmp_path):
    ...
    for key, markup in doc._designs.items():
        assert card_frame.centring_shift(markup, CARD) == (0.0, 0.0), key
