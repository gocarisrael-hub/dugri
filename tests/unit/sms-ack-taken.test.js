...
describe('reporting the batch', () => {
  it('marks everything this batch is holding as sent', () => {
    const a = queue('c1');
    const b = queue('c2');
    const { batch, messages } = sms.claim({ now: NOW });
    expect(messages).toHaveLength(2);

    const { known, sent } = sms.ackTaken({ batch, now: NOW + 1000 });
  ...
  it('gives one token to the whole poll, and a fresh one to the next', () => {
    ...
    expect(second.batch).toBeTruthy();
  ...
  it('is a no-op when the batch is holding nothing, rather than an error', () => {
    ...
    sms.ackTaken({ batch, now: NOW + 1000 });
    const again = sms.ackTaken({ batch, now: NOW + 2000 });
  ...
  it('never touches a message that is still waiting its turn', () => {
    const a = queue('c1');
    const { batch } = sms.claim({ now: NOW, limit: 1 });
    const b = queue('c2', NOW + 1000);
    sms.ackTaken({ batch, now: NOW + 2000 });
...
describe('a report settles its own batch and nothing else', () => {
  const twoBatches = () => {
    ...
    return {
      ...
      a,
      b,
      aIds: a.messages.map((m) => m.id),
      bIds: b.messages.map((m) => m.id),
    };
  };
  ...
  it('leaves the batch whose response was lost untouched', () => {
    ...
    const { sent } = sms.ackTaken({ batch: b.batch, now: NOW + 1000 });
  ...
  it('does not let an earlier run settle a later one', () => {
    ...
    sms.ackTaken({ batch: a.batch, now: NOW + 1000 });
  ...
  it('is stale once the message has been re-leased to a later batch', () => {
    ...
    expect(second.messages.map((x) => x.id)).toEqual([m.id]);

    const r = sms.ackTaken({ batch: first.batch, now: later + 1000 });
    expect(r).toMatchObject({ known: false });
    expect(stateOf(m.id, later + 1000).state).toBe('taken');

    sms.ackTaken({ batch: second.batch, now: later + 2000 });
...
describe('what it must not swallow', () => {
  ...
  it('leaves a message whose lease already ran out in the queue', () => {
    ...
    expect(sms.ackTaken({ batch, now: late }).sent).toEqual([]);
  ...
  it('does not revive a message that was given up on', () => {
    ...
    expect(sms.ackTaken({ batch, now: NOW + 2000 }).sent).toEqual([]);
  ...
  it('does not re-date a message already reported one by one', () => {
    ...
    expect(sms.ackTaken({ batch, now: NOW + 5000 }).sent).toEqual([]);
...
describe('alongside the per-message report', () => {
  it('settles only what is left after some were reported singly', () => {
    ...
    const { sent } = sms.ackTaken({ batch, now: NOW + 1000 });
    expect(sent.map((m) => m.id)).toEqual([b.id]);
    expect(sms.counts(NOW + 1000)).toMatchObject({ sent: 2, taken: 0, pending: 0 });
  });
});

describe('a poll with nothing in it', () => {
  // The phone reports from one block with no condition in front of it, so the
  // quiet cycles — most of them — hit this. An empty batch is one we issued and
  // one we can answer for: nothing to settle, and nothing wrong.
  it('still names a batch, and reporting it is a clean no-op', () => {
    const empty = sms.claim({ now: NOW });
    expect(empty.messages).toEqual([]);
    expect(empty.batch).toEqual(expect.any(String));
    const r = sms.ackTaken({ batch: empty.batch, now: NOW + 1000 });
    expect(r).toMatchObject({ known: true });
    expect(r.sent).toEqual([]);
  });

  // …and it is not a skeleton key: an empty batch settles nothing that another
  // poll is holding.
  it('settles nothing another batch is holding', () => {
    const m = queue('c1');
    const held = sms.claim({ now: NOW });
    const empty = sms.claim({ now: NOW });
    expect(empty.messages).toEqual([]);
    expect(sms.ackTaken({ batch: empty.batch, now: NOW + 1000 }).sent).toEqual([]);
    expect(stateOf(m.id, NOW + 1000).state).toBe('taken');
    expect(sms.ackTaken({ batch: held.batch, now: NOW + 2000 }).sent).toHaveLength(1);
  });
});
