...
// FEATURES.color_picking is on (with colour off the step is not in the flow at
// all). Detection keys on the literal data-step id 2, so renumbering the step
// means updating goStep too.
...
test.describe('preview tabs hidden on the colour step', () => {
  ...
  test('with the colour flag OFF, step 2 is skipped and step 1 keeps its tabs', async ({
    ...
    // colour off -> step 2 drops out entirely. is-step-color is never set, so
    // the design-step tabs stay visible (no false hide).
    await stubFeatures(page, { color_picking: false });
    await page.goto('/options.html?plan=base');
