...
// Agent A's store blocks (server/stores/commerce.js), destructured and spread below.
const commerceStore = require('./stores/commerce');
...
// Agent A: ORDER_PRICES, the live pricing reads, the copy cap and the delivery fee,
// in server/stores/commerce.js.
const {
  ORDER_PRICES,
  versionEnabled,
  versionPrice,
  ...
  MAX_COPIES,
  deliveryFee,
} = commerceStore.pricing({ settings });
...
// Agent A: the Meta report timing constants and helpers, in server/stores/commerce.js.
const {
  ...
  dropMetaCtx,
  withoutMetaCtx,
} = commerceStore.metaReportHelpers();
...
// Agent A: the opening stock counts, in server/stores/commerce.js.
const { INITIAL_BOARD_STOCK, SUPPLY_DEFAULTS, stockInt } = commerceStore.stockHelpers();

// Agent A: the copy-count sanitiser and the authoritative order total, in
// server/stores/commerce.js.
const { sanitizeQuantity, orderTotal } = commerceStore.orderTotals({
  MAX_COPIES,
  versionPrice,
  deliveryFee,
});
...
// Agent A: the boot-time Meta report migration, in server/stores/commerce.js.
const { migrateMetaReports } = commerceStore.metaReportMigration({ _db, saveDb });
...
// Agent A: the coupon money helpers, in server/stores/commerce.js.
const { round2, normalizeCommission, normMaxUses, couponSpent, commissionFor } =
  commerceStore.couponHelpers({ sanitizeQuantity });
...
const db = {
  ...
  // Agent A: the Meta Conversions API report state, in server/stores/commerce.js.
  ...commerceStore.metaReport({
    _db,
    saveDb,
    nowIso,
    claimable,
  ...
  // Agent A: discount and partner coupons, in server/stores/commerce.js.
  ...commerceStore.coupons({
    _db,
    saveDb,
    crypto,
    uid,
  ...
  // Agent A: physical stock, in server/stores/commerce.js.
  ...commerceStore.stock({
    _db,
    saveDb,
    nowIso,
