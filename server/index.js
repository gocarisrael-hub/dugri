...
// Agent A's routes (server/routes/commerce.js), registered in place below.
...
// Agent A: the sticker sheet and the physical stock, in server/routes/commerce.js.
// The sticker helpers come back for module.exports (unit tests), and stockDesigns
// for applyOrderReady below.
...
// Agent A: the ready-batch press and the HFD courier routes, in server/routes/commerce.js.
...
// Agent A: the admin coupon routes and the partner report, in server/routes/commerce.js.
...
// Agent A: pay/cancel, pay/init, shipping/init and the PeleCard callback, in
// server/routes/commerce.js.
//
// metaAdContext, metaCapiArmed and sendPurchaseToMeta are the functions D's ads
// block returns, and that block registers further down this file. Their const
// bindings do not exist yet at this line, so they are handed over as call-time
// forwarders: every request still reaches the one function D's block returned,
// exactly as it did when these routes sat inline here.
commerceRoutes.registerPayment(app, {
  pelecard,
  crypto,
  db,
  paymentBaseUrl,
  ...
  onOrderPaid,
  onShippingAdded,
  metaAdContext: (...args) => metaAdContext(...args),
  metaCapiArmed: (...args) => metaCapiArmed(...args),
  sendPurchaseToMeta: (...args) => sendPurchaseToMeta(...args),
});
