...
// Agent D's non-route machinery (server/platform-hooks.js): the notification
// hooks, the WhatsApp bot and the reminder schedulers. Built below, at the spot
// the first of those blocks used to sit.
const platformHooks = require('./platform-hooks');
// Agent B's routes (server/routes/catalog.js), registered the same way.
const catalogRoutes = require('./routes/catalog');
// Agent A's routes (server/routes/commerce.js), registered the same way.
...
// Agent D: the notification hooks, the WhatsApp bot machinery and the reminder
// schedulers, in server/platform-hooks.js (slice 1b). Everything below calls the
// returned functions exactly as it called the inline ones.
//
// It is built HERE rather than where those blocks used to sit, and the position
// is load-bearing in both directions. The inline versions were function
// DECLARATIONS, hoisted, so two register calls could read them as values from
// above: registerOrder is handed onOrderCreated, and registerAdsSettingsWhatsapp
// is handed handleWaEvent / openWhatsappGroup / ilPhoneToWaId /
// resolveProductImageUrl. A `const` is not hoisted, so built at the old spot
// those reads hit the temporal dead zone and the app throws at require time.
// The floor is ADMIN_KEY just above (the last dependency); the ceiling is that
// first register call. Everything else that uses these names does so inside a
// handler, which runs long after this line.
const {
...
// Agent A: the ready-batch press and the HFD courier routes, in
// server/routes/commerce.js.
commerceRoutes.registerReadyBatchAndHfd(app, {
  requireAdmin,
  db,
  sms,
  hfd,
...
// Agent A: the admin coupon routes and the partner report, in
// server/routes/commerce.js.
commerceRoutes.registerCoupons(app, { requireAdmin, db });
...
// Agent A: the checkout's coupon preview, in server/routes/commerce.js.
commerceRoutes.registerCouponValidate(app, { db, couponRateOk, clientKey });
...
// Agent A: setting the order, in server/routes/commerce.js.
commerceRoutes.registerOrder(app, { db, onOrderCreated, paymentBaseUrl });
...
// Agent A: the public celebrations counter, in server/routes/commerce.js.
commerceRoutes.registerStatsOrders(app, { db });
...
// Agent A: the public pricing reads, in server/routes/commerce.js.
commerceRoutes.registerPricing(app, { db });
